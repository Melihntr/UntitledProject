...
class CheckSuspiciousTransfersHandlerTest {
    ...
    @Test
    void handle_returnsEmptyList_whenPortReturnsNull() {
        when(transactionPort.getSuspiciousTransfers()).thenReturn(null);

        assertThat(handler.handle()).isEmpty();

        verify(transactionPort).getSuspiciousTransfers();
    }
}
