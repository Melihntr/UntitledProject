...
class CreateUserHandlerTest {
    ...
    @Test
    void handle_keepsProvidedSeederId() {
        UserCreateInput input = UserCreateInput.builder()
                .id("static-id")
                .username("seed")
        ...
        when(userPort.save(org.mockito.ArgumentMatchers.any(UserModel.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));

        UserModel result = handler.handle(input);

        assertThat(result.getId()).isEqualTo("static-id");
        verify(eventPublisherPort).publishUserCreated("static-id");
    }
}
